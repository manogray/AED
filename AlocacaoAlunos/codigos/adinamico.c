...
static void AtualizarOcupa(TArrayDinamico* vet){
    TDadosADI* dadoVet = vet->dadosADI;
    int contador = 0;

    while(contador < dadoVet->tamanho){
        TArrayDinamico* vetInterno = dadoVet->vetor[contador];
        TDadosADI* dadoVetInterno = vetInterno->dadosADI;
        dadoVet->ocupacao = dadoVet->ocupacao + dadoVetInterno->ocupacao;
...
TArrayDinamico *criarADI(int tamanhoInicial){
	...
	if(adi != NULL){
		...
		adi->atualizarOcupacao = AtualizarOcupa;
